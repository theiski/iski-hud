...
function IsInSafeZone()
    ...
    for _, iZone in pairs(SafeZoneList) do
        local InZone = iZone:isPointInside(coord)
        if InZone then
            SafeZone = true
            return
        end
...
local function hasHarness(items)
    -- Check if jim-mechanic is available
    if GetResourceState('jim-mechanic') ~= 'missing' then
        -- Try to use jim-mechanic harness
        ...
        if success then
            ...
            harness = 0 -- Default value if function fails
        end
    else
        -- Fallback if jim-mechanic is not available
        harness = 0
    end
end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    Wait(2000)
    TriggerServerEvent('qb_hud:RequestPlaylists')
...
local prevPlayerStats = { nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil }
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        -- Check which fuel resource is available and use it
        if GetResourceState('ps-fuel') ~= 'missing' then
            -- Try to use ps-fuel
            local success, result = pcall(function()
            ...
            if success then
                lastFuelCheck = result
            else
                lastFuelCheck = 100 -- Default value if function fails
            end
        elseif GetResourceState('LegacyFuel') ~= 'missing' then
            -- Try to use LegacyFuel
            local success, result = pcall(function()
            ...
            -- Fallback if no fuel resource is available
                ...
                if IsPedInAnyVehicle(ped, false) then
                    ...
                    if config.VehClassStress[tostring(vehClass)] and not config.WhitelistedVehicles[vehHash] then
                        ...
                        if speed >= stressSpeed then
                            TriggerServerEvent('hud:server:GainStress', math.random(1, 3))                        end
                    end
                end
            end
            Wait(10000)
...
local isRadio = false
local voiceMode = 2 -- Default to normal mode (1 = whisper, 2 = normal, 3 = shouting)
...
-- Create a thread to check talking state
CreateThread(function()
    while true do
        ...
        if isTalking ~= talking then -- Only update if state changed
            isTalking = talking
            SendNUIMessage({
                action = "updateVoiceState",
                isTalking = isTalking,
...
-- Radio Talking State Handler
AddEventHandler('pma-voice:radioActive', function(radioTalking)
    isRadio = radioTalking
    SendNUIMessage({
        action = "updateVoiceState",
        isTalking = isTalking,
        isRadio = isRadio,
